###############################################################################
# Rebuild syn21788402 EC stepH Seurat object from raw SCE
#
# Output:
...
out_dir <- Sys.getenv(
  "SYN21788402_EC_OUT_DIR",
  "D:/codex/21788402/redo/EC"
)
sce_file <- file.path(raw_dir, "sce.EC.scAlign.assigned.rds")
out_file <- file.path(
  out_dir,
  "stepH_syn21788402_EC_obj_labeled_celltype7_celltype6.rds"
)
...
# ---- Build EC object ----------------------------------------------------------
cat("\n============================================================\n")
cat("Processing syn21788402 EC\n")
cat("Input file : ", sce_file, "\n", sep = "")
cat("Output file: ", out_file, "\n", sep = "")
cat("============================================================\n")
if (!file.exists(sce_file)) stop("Cannot find EC input file: ", sce_file)
...
if (!"counts" %in% dat$assay_names) stop("EC: SCE has no assay 'counts'")
if (!"logcounts" %in% dat$assay_names) stop("EC: SCE has no assay 'logcounts'")
if (!"CCA.ALIGNED" %in% dat$reduced_dim_names) {
  stop("EC: SCE has no reducedDim 'CCA.ALIGNED'")
}

required_meta <- c("clusterCellType", "clusterAssignment", "SampleID")
missing_meta <- setdiff(required_meta, colnames(dat$meta))
if (length(missing_meta) > 0) {
  stop("EC: missing colData columns: ", paste(missing_meta, collapse = ", "))
}
...
if (!identical(colnames(counts_mat), rownames(meta_df))) {
  stop("EC: counts colnames and metadata rownames differ")
}
if (!identical(colnames(counts_mat), colnames(logcounts_mat))) {
  stop("EC: counts and logcounts cell names differ")
}
if (!identical(rownames(counts_mat), rownames(logcounts_mat))) {
  stop("EC: counts and logcounts gene names differ")
}
...
if (anyDuplicated(gene_names_clean)) {
  cat("EC: duplicate gene names after '_' -> '-', applying make.unique()\n")
...
if (any(is.na(short))) {
  ...
  stop("EC: unmapped clusterCellType values: ", paste(bad, collapse = ", "))
}
...
cat("\nclusterCellType x celltype6 for EC:\n")
print(table(obj$clusterCellType, obj$celltype6))
cat("\ncelltype6 counts for EC:\n")
print(table(obj$celltype6))

expected_counts <- c(
  "Astrocytes" = 5916,
  "Excitatory neurons" = 10780,
  "Microglia" = 6003,
  "Endothelial" = 730,
  "Inhibitory neurons" = 5571,
  "Oligodendrocytes" = 13528
)
...
if (!identical(as.integer(got_counts[names(expected_counts)]), as.integer(expected_counts))) {
  stop("EC celltype6 counts do not match expected corrected mapping")
}

stopifnot(nrow(obj) == 33694)
stopifnot(ncol(obj) == 42528)
...
write_celltype_qc(obj, "EC", out_dir)

session_file <- file.path(out_dir, "sessionInfo_stepH_syn21788402_EC_rebuild.txt")
sink(session_file)
print(sessionInfo())
sink()
